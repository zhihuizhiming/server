...
class LoginController extends Controller
{
     ...
     */
    function __construct($appName, IRequest $request, IUserManager $userManager, IConfig $config, ISession $session, IUserSession $userSession, IURLGenerator $urlGenerator, Manager $twoFactorManager, Throttler $throttler)
    {
    ...
    public function tryLogin($user, $password, $redirect_url, $remember_login = false, $timezone = '', $timezone_offset = '')
    {
        ...
        if ($this->twoFactorManager->isTwoFactorAuthenticated($loginResult)) {
            ...
            if (count($providers) === 1) {
                ...
                $urlParams = ['challengeProviderId' => $provider->getId()];
            } else {
