...
abstract class StoragesController extends Controller
{
    ...
    protected function updateStorageStatus(StorageConfig &$storage, $testOnly = true)
    {
        try {
            ...
            $storage->setStatus(\OC_Mount_Config::getBackendStatus($backend->getStorageClass(), $storage->getBackendOptions(), false, $testOnly));
        } catch (InsufficientDataForMeaningfulAnswerException $e) {
            $status = $e->getCode() ? $e->getCode() : StorageNotAvailableException::STATUS_INDETERMINATE;
            $storage->setStatus($status, $this->l10n->t('Insufficient data: %s', [$e->getMessage()]));
        } catch (StorageNotAvailableException $e) {
            $storage->setStatus($e->getCode(), $this->l10n->t('%s', [$e->getMessage()]));
        } catch (\Exception $e) {
    ...
    public function index()
    {
        ...
        return new DataResponse($storages, Http::STATUS_OK);
    }
    /**
