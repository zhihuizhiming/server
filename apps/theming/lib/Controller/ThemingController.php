...
class ThemingController extends Controller
{
     ...
     */
    public function __construct($appName, IRequest $request, IConfig $config, ThemingDefaults $template, Util $util, ITimeFactory $timeFactory, IL10N $l, IRootFolder $rootFolder, ITempManager $tempManager)
    {
    ...
    public function updateStylesheet($setting, $value)
    {
        ...
        switch ($setting) {
            case 'name':
                if (strlen($value) > 250) {
                    return new DataResponse(['data' => ['message' => $this->l->t('The given name is too long')], 'status' => 'error']);
                }
                break;
            case 'url':
                if (strlen($value) > 500) {
                    return new DataResponse(['data' => ['message' => $this->l->t('The given web address is too long')], 'status' => 'error']);
                }
                break;
            case 'slogan':
                if (strlen($value) > 500) {
                    return new DataResponse(['data' => ['message' => $this->l->t('The given slogan is too long')], 'status' => 'error']);
                }
                break;
            case 'color':
                if (!preg_match('/^\\#([0-9a-f]{3}|[0-9a-f]{6})$/i', $value)) {
                    return new DataResponse(['data' => ['message' => $this->l->t('The given color is invalid')], 'status' => 'error']);
                }
                break;
        }
        $this->template->set($setting, $value);
        return new DataResponse(['data' => ['message' => $this->l->t('Saved')], 'status' => 'success']);
    }
    /**
    ...
    public function updateLogo()
    {
        ...
        if (empty($newLogo) && empty($newBackgroundLogo)) {
            return new DataResponse(['data' => ['message' => $this->l->t('No file uploaded')]], Http::STATUS_UNPROCESSABLE_ENTITY);
        }
        $name = '';
        if (!empty($newLogo)) {
        ...
        if (!empty($newBackgroundLogo)) {
            ...
            if ($image === false) {
                return new DataResponse(['data' => ['message' => $this->l->t('Unsupported image type')], 'status' => 'failure'], Http::STATUS_UNPROCESSABLE_ENTITY);
            }
        ...
        return new DataResponse(['data' => ['name' => $name, 'message' => $this->l->t('Saved')], 'status' => 'success']);
    }
    /**
    ...
    public function undo($setting)
    {
        ...
        return new DataResponse(['data' => ['value' => $value, 'message' => $this->l->t('Saved')], 'status' => 'success']);
    }
    /**
     * @PublicPage
     * @NoCSRFRequired
    ...
    public function getStylesheet()
    {
        ...
        if ($color !== '') {
            $responseCss .= sprintf('#body-user #header,#body-settings #header,#body-public #header,#body-login,.searchbox input[type="search"]:focus,.searchbox input[type="search"]:active,.searchbox input[type="search"]:valid {background-color: %s}' . "\n", $color);
            $responseCss .= sprintf('input[type="checkbox"].checkbox:checked:enabled:not(.checkbox--white) + label:before {' . 'background-image:url(\'%s/core/img/actions/checkmark-white.svg\');' . 'background-color: %s; background-position: center center; background-size:contain;' . 'width:12px; height:12px; padding:0; margin:2px 6px 6px 2px; border-radius:1px;' . "}\n", \OC::$WEBROOT, $elementColor);
            $responseCss .= 'input[type="radio"].radio:checked:not(.radio--white):not(:disabled) + label:before {' . 'background-image: url(\'data:image/svg+xml;base64,' . $this->util->generateRadioButton($elementColor) . '\');' . "}\n";
