...
class MountPublicLinkController extends Controller
{
     ...
     */
    public function __construct($appName, IRequest $request, FederatedShareProvider $federatedShareProvider, IManager $shareManager, AddressHandler $addressHandler, ISession $session, IL10N $l, IUserSession $userSession, IClientService $clientService)
    {
    ...
    public function createFederatedShare($shareWith, $token, $password = '')
    {
        ...
        if (!$this->federatedShareProvider->isOutgoingServer2serverShareEnabled()) {
            return new JSONResponse(['message' => 'This server doesn\'t support outgoing federated shares'], Http::STATUS_BAD_REQUEST);
        }
        try {
        ...
        $authenticated = $this->session->get('public_link_authenticated') === $share->getId() || $this->shareManager->checkPassword($share, $password);
    ...
    {
        $name = $_GET['name'];
        $ownerDisplayName = $_GET['ownerDisplayName'];
        $owner = $_GET['owner'];
        $password = $_GET['password'];
        $remote = $_GET['remote'];
        $token = $_GET['token'];
        ...
        try {
            $response = $httpClient->post($remote . '/index.php/apps/federatedfilesharing/createFederatedShare', ['body' => ['token' => $token, 'shareWith' => rtrim($shareWith, '/'), 'password' => $password], 'connect_timeout' => 10]);
        } catch (\Exception $e) {
            if (empty($password)) {
    ...
    private function legacyMountPublicLink($token, $remote, $password, $name, $owner, $ownerDisplayName)
    {
        ...
        $externalManager = new \OCA\Files_Sharing\External\Manager(\OC::$server->getDatabaseConnection(), \OC\Files\Filesystem::getMountManager(), \OC\Files\Filesystem::getLoader(), \OC::$server->getHTTPClientService(), \OC::$server->getNotificationManager(), $discoveryManager, \OC::$server->getUserSession()->getUser()->getUID());
        // check for ssl cert
        if (substr($remote, 0, 5) === 'https') {
            try {
                $client = $this->clientService->newClient();
                $client->get($remote, ['timeout' => 10, 'connect_timeout' => 10])->getBody();
            } catch (\Exception $e) {
        ...
        try {
            ...
            \OCP\Util::writeLog('federatedfilesharing', 'Invalid remote storage: ' . get_class($e) . ': ' . $e->getMessage(), \OCP\Util::DEBUG);
        ...
        if ($result) {
            try {
                ...
                return new JSONResponse(['message' => $this->l->t('Federated Share successfully added'), 'legacyMount' => '1']);
            } catch (\OCP\Files\StorageInvalidException $e) {
                \OCP\Util::writeLog('federatedfilesharing', 'Invalid remote storage: ' . get_class($e) . ': ' . $e->getMessage(), \OCP\Util::DEBUG);
                return new JSONResponse(['message' => $this->l->t('Storage not valid')], Http::STATUS_BAD_REQUEST);
            } catch (\Exception $e) {
            ...
            $externalManager->removeShare($mount->getMountPoint());
            \OCP\Util::writeLog('federatedfilesharing', 'Couldn\'t add remote share', \OCP\Util::DEBUG);
            return new JSONResponse(['message' => $this->l->t('Couldn\'t add remote share')], Http::STATUS_BAD_REQUEST);
        }
    }
}