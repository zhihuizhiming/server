...
class ControllerTest extends \Test\TestCase
{
    ...
    {
        parent::setUp();
        $request = new Request(['get' => ['name' => 'John Q. Public', 'nickname' => 'Joey'], 'post' => ['name' => 'Jane Doe', 'nickname' => 'Janey'], 'urlParams' => ['name' => 'Johnny Weissmüller'], 'files' => ['file' => 'filevalue'], 'env' => ['PATH' => 'daheim'], 'session' => ['sezession' => 'kein'], 'method' => 'hi'], $this->getMockBuilder('\\OCP\\Security\\ISecureRandom')->disableOriginalConstructor()->getMock(), $this->getMockBuilder('\\OCP\\IConfig')->disableOriginalConstructor()->getMock());
        $this->app = $this->getMockBuilder('OC\\AppFramework\\DependencyInjection\\DIContainer')->setMethods(['getAppName'])->setConstructorArgs(['test'])->getMock();
        $this->app->expects($this->any())->method('getAppName')->will($this->returnValue('apptemplate_advanced'));
    ...
    {
        $params = array('name' => 'Johnny Weissmüller', 'nickname' => 'Janey');
    ...
    {
        $responder = $this->controller->getResponderByHTTPHeader('*/*, application/tom, application/json');
