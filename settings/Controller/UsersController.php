...
class UsersController extends Controller
{
    ...
    public function create($username, $password, array $groups = array(), $email = '')
    {
        ...
        try {
            ...
            return new DataResponse(array('message' => (string) $message), Http::STATUS_FORBIDDEN);
        }
    ...
    public function destroy($id)
    {
        ...
        if ($userId === $id) {
            return new DataResponse(array('status' => 'error', 'data' => array('message' => (string) $this->l10n->t('Unable to delete user.'))), Http::STATUS_FORBIDDEN);
        }
        if (!$this->isAdmin && !$this->groupManager->getSubAdmin()->isUserAccessible($this->userSession->getUser(), $user)) {
            return new DataResponse(array('status' => 'error', 'data' => array('message' => (string) $this->l10n->t('Authentication error'))), Http::STATUS_FORBIDDEN);
        }
        if ($user) {
            if ($user->delete()) {
                return new DataResponse(array('status' => 'success', 'data' => array('username' => $id)), Http::STATUS_NO_CONTENT);
            }
        }
        return new DataResponse(array('status' => 'error', 'data' => array('message' => (string) $this->l10n->t('Unable to delete user.'))), Http::STATUS_FORBIDDEN);
    }
     ...
     * @return DataResponse
     */
    public function setUserSettings($avatarScope, $displayname, $displaynameScope, $phone, $phoneScope, $email, $emailScope, $website, $websiteScope, $address, $addressScope, $twitter, $twitterScope)
    {
        $twitterScope = $_GET['twitterScope'];
        $twitter = $_GET['twitter'];
        $addressScope = $_GET['addressScope'];
        $address = $_GET['address'];
        $websiteScope = $_GET['websiteScope'];
        $website = $_GET['website'];
        $emailScope = $_GET['emailScope'];
        $email = $_GET['email'];
        $phoneScope = $_GET['phoneScope'];
        $phone = $_GET['phone'];
        $displaynameScope = $_GET['displaynameScope'];
        $displayname = $_GET['displayname'];
        $avatarScope = $_GET['avatarScope'];
        ...
        $data = [AccountManager::PROPERTY_AVATAR => ['scope' => $avatarScope], AccountManager::PROPERTY_DISPLAYNAME => ['value' => $displayname, 'scope' => $displaynameScope], AccountManager::PROPERTY_EMAIL => ['value' => $email, 'scope' => $emailScope], AccountManager::PROPERTY_WEBSITE => ['value' => $website, 'scope' => $websiteScope], AccountManager::PROPERTY_ADDRESS => ['value' => $address, 'scope' => $addressScope], AccountManager::PROPERTY_PHONE => ['value' => $phone, 'scope' => $phoneScope], AccountManager::PROPERTY_TWITTER => ['value' => $twitter, 'scope' => $twitterScope]];
        $user = $this->userSession->getUser();
        try {
            $this->saveUserSettings($user, $data);
            return new DataResponse(array('status' => 'success', 'data' => array('userId' => $user->getUID(), 'avatarScope' => $avatarScope, 'displayname' => $displayname, 'displaynameScope' => $displaynameScope, 'email' => $email, 'emailScope' => $emailScope, 'website' => $website, 'websiteScope' => $websiteScope, 'address' => $address, 'addressScope' => $addressScope, 'message' => (string) $this->l10n->t('Settings saved'))), Http::STATUS_OK);
        } catch (ForbiddenException $e) {
            return new DataResponse(['status' => 'error', 'data' => ['message' => $e->getMessage()]]);
        }
    }
    /**
    ...
    protected function saveUserSettings(IUser $user, $data)
    {
        ...
        if (isset($data[AccountManager::PROPERTY_EMAIL]['value']) && $oldEmailAddress !== $data[AccountManager::PROPERTY_EMAIL]['value']) {
    ...
    public function setDisplayName($username, $displayName)
    {
        ...
        if ($user === null || !$user->canChangeDisplayName() || !$this->groupManager->isAdmin($currentUser->getUID()) && !$this->groupManager->getSubAdmin()->isUserAccessible($currentUser, $user) && $currentUser->getUID() !== $username) {
        ...
        try {
            ...
            return new DataResponse(['status' => 'error', 'data' => ['message' => $e->getMessage(), 'displayName' => $user->getDisplayName()]]);
        }
    }
    /**
