...
class ChangePasswordController extends Controller
{
     ...
     * @param IL10N $l
     */
    public function __construct($appName, IRequest $request, $userId, IUserManager $userManager, IUserSession $userSession, IGroupManager $groupManager, IAppManager $appManager, IL10N $l)
    {
    ...
    public function changePersonalPassword($oldpassword = '', $newpassword = null)
    {
        ...
        if ($user === false) {
            return new JSONResponse(['status' => 'error', 'data' => ['message' => $this->l->t('Wrong password')]]);
        }
        try {
            ...
            return new JSONResponse(['status' => 'error', 'data' => ['message' => $e->getHint()]]);
        }
        $this->userSession->updateSessionTokenPassword($newpassword);
        return new JSONResponse(['status' => 'success', 'data' => ['message' => $this->l->t('Saved')]]);
    }
    /**
     * @NoAdminRequired
    ...
    {
        $recoveryPassword = $_GET['recoveryPassword'];
        $password = $_GET['password'];
        $username = $_GET['username'];
        if ($username === null) {
            return new JSONResponse(['status' => 'error', 'data' => ['message' => $this->l->t('No user supplied')]]);
        }
        if ($password === null) {
            return new JSONResponse(['status' => 'error', 'data' => ['message' => $this->l->t('Unable to change password')]]);
        }
        $currentUser = $this->userSession->getUser();
        $targetUser = $this->userManager->get($username);
        if ($currentUser === null || $targetUser === null || !($this->groupManager->isAdmin($this->userId) || $this->groupManager->getSubAdmin()->isUserAccessible($currentUser, $targetUser))) {
            return new JSONResponse(['status' => 'error', 'data' => ['message' => $this->l->t('Authentication error')]]);
        }
        if ($this->appManager->isEnabledForUser('encryption')) {
            ...
            if ($recoveryEnabledForUser && $recoveryPassword === '') {
                return new JSONResponse(['status' => 'error', 'data' => ['message' => $this->l->t('Please provide an admin recovery password, otherwise all user data will be lost')]]);
            } elseif ($recoveryEnabledForUser && !$validRecoveryPassword) {
                return new JSONResponse(['status' => 'error', 'data' => ['message' => $this->l->t('Wrong admin recovery password. Please check the password and try again.')]]);
            } else {
                ...
                if (!$result && $recoveryEnabledForUser) {
                    return new JSONResponse(['status' => 'error', 'data' => ['message' => $this->l->t('Backend doesn\'t support password change, but the user\'s encryption key was successfully updated.')]]);
